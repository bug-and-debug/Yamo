...
- (void) setDefaultValues
{
    ...
    //_topImageView.layer.cornerRadius = _topImageView.frame.size.width/2.f;
    //_bottomImageView.layer.cornerRadius = _bottomImageView.frame.size.width/2.f;
    
    //_topImageView.layer.borderWidth = 10.f;
    //_topImageView.layer.borderColor = [COLOR_GRAY CGColor];
    
    //_bottomImageView.layer.borderWidth = 10.f;
    //_bottomImageView.layer.borderColor = [COLOR_GRAY CGColor];
...
- (void) populateImageView:(UIImageView*) imageView andIndex:(NSUInteger) index
{
    ...
    } else if ([dataObj isKindOfClass:[NSString class]])
    {
        ...
        //[[Utils sharedObject] loadImageFromServerAndLocalWithoutRound:imageName imageView:imageView];
